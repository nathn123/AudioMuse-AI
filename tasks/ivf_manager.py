...
from config import EMBEDDING_DIMENSION, INDEX_NAME, INDEX_NAME_MAEST, IVF_METRIC, FUSION_WEIGHT_MUSICNN_DEFAULT, MAX_SONGS_PER_ARTIST, DUPLICATE_DISTANCE_THRESHOLD_COSINE, DUPLICATE_DISTANCE_THRESHOLD_EUCLIDEAN, DUPLICATE_DISTANCE_CHECK_LOOKBACK, MOOD_SIMILARITY_THRESHOLD, SIMILARITY_ELIMINATE_DUPLICATES_DEFAULT, SIMILARITY_RADIUS_DEFAULT, MOOD_SIMILARITY_ENABLE, IVF_RESULT_CACHE_SECONDS, IVF_RESULT_CACHE_MAX, RADIUS_INSTRUMENTATION, IVF_RERANK_OVERFETCH
...
# --- MAEST index globals (parallel to the musicnn index above) ---
...
def find_nearest_neighbors_fused(
    ...
    # Query both indexes (overfetch by 2× for good fusion coverage)
    ...
    for item_id, data in fused.items():
        # Normalize each stream's distances to [0, 1] then fuse
